...
class Key_check(QMainWindow, Key_input_Ui_Form) :
    ...
    def check_key(self) :
        ...
        # print(text)
        if text == "qlalfqjsgh" or text == "alsrl":
            print("open")
            self.title_class()
        else : 
            print("fall")
...
class ppimppie_title(QMainWindow, ppimppie_title_Ui_Form) : 
    def __init__(self) :
        ...
        self.befo_btn.clicked.connect(self.key_check)
        self.dos_btn.clicked.connect(self.Dos)
        self.w_shall_btn.clicked.connect(self.webshall)
        self.exit_btn.clicked.connect(self.exit)
        self.pushButton_4.clicked.connect(self.notyet)
        self.pushButton_5.clicked.connect(self.notyet)

    def key_check(self) :
        self.befo = Key_check_for_bruteforce()
        self.befo.show()
    ...
    def notyet(self) :
        QMessageBox.information(self, "데모버전", "아직 구현되지 않은 기능입니다.")

# ============================타이틀 화면==============================
# ====================================================================
class dos_choice(QMainWindow, Dosat_Ui_Form):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
...
class Dos(QMainWindow, Dos_Ui_Form):
    def __init__(self):
        ...
        # self.next_btn.clicked.connect(self.notyet)        # 잠금
        self.next_btn.clicked.connect(self.start_attack)        # 해제
...
class p_o_d(QMainWindow, Dos_Ui_Form):
    def __init__(self):
        ...
        # self.attack_btn.clicked.connect(self.notyet)        # 잠금
        self.attack_btn.clicked.connect(self.main)        # 해제
...
class Land_attack(QMainWindow, Dos_Ui_Form):
    def __init__(self):
        ...
        # self.attack_btn_2.clicked.connect(self.notyet)        # 잠금
        self.attack_btn_2.clicked.connect(self.main)        # 잠금
...
class SynFloodThread(QThread):
    ...
    def stop(self):
        self.running = False
class Syn_flooding(QMainWindow, Dos_Ui_Form):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        self.stackedWidget.setCurrentIndex(4)
        # self.attack_btn_3.clicked.connect(self.notyet)        # 잠금
        self.attack_btn_3.clicked.connect(self.main)        # 잠금
...
class SmurfAttackThread(QThread):
    ...
    def stop(self):
        self.running = False
class Smurf(QMainWindow, Dos_Ui_Form):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
        ...
        # self.attack_btn_4.clicked.connect(self.notyet)        # 잠금
        self.attack_btn_4.clicked.connect(self.main)        # 헤제
...
class attck_result1(QMainWindow, Ui_Form):
    def __init__(self):
        super().__init__()
        self.setupUi(self)
...
# ========================================= 
class Webshall(QMainWindow, web_shall_Ui_Form):
    def __init__(self) :
        super().__init__()
        self.setupUi(self)
# ========================================= 
...
# # 실행 경로 설정 (PyInstaller 환경과 일반 환경 모두 지원)
